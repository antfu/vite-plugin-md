...
import type { PipeTask, Pipeline, PipelineStage, ResolvedOptions } from '../types'
...
const getBuilders = <
  ...
  O extends ResolvedOptions<readonly ConfiguredBuilder<string, BuilderOptions, PipelineStage, string>[]>,
>(
    stage: S,
    options: O,
  ) => {
  const builders = options.builders.reduce(
    (acc, b) => {
      return b.about.stage === stage ? [...acc, b() as unknown as BuilderRegistration<BuilderOptions, S>] : acc
    },
  ...
  return (payload: Pipeline<S, B>) => {
    ...
    const asyncApi = async (payload: Pipeline<S, B>): Promise<Pipeline<S, B>> => {
      for (const b of builders) {
        try {
          payload = (await b.handler(payload as any, b.options)) as unknown as Pipeline<S, B>
        }
        catch (e) {
...
<S extends PipelineStage>(stage: S) => {
  const task = (payload: PipeTask<S, B>): PipeTask<S, B> => {
    const bt = getBuilderTask<S, B>(stage, options)
    return pipe(
      payload,
      TE.map(bt),
      TE.flatten,
